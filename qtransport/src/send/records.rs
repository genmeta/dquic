...
/// packet assembly, encryption and socket submission never borrow a journal guard.
#[derive(Clone, Default)]
pub struct ArcSendJournal(Arc<Mutex<SendJournal>>);

impl ArcSendJournal {
    #[cfg(test)]
    pub(crate) fn starting_at(pn: u64) -> Self {
        Self(Arc::new(Mutex::new(SendJournal {
            next_pn: pn,
            ..Default::default()
        })))
    }
    ...
    pub(crate) fn record_pending(
        ...
        // Path descriptors stay with the Sender until socket submission succeeds.
        for frame in frames.extract_if(.., |frame| {
            !matches!(frame, Frame::PathChallenge(_) | Frame::PathResponse(_))
        }) {
            if let Ok(frame) = GuaranteedFrame::try_from(frame) {
    ...
    pub fn mark_sent(
        ...
        if in_flight {
            ...
            records.deadlines.insert((retrans_at, pn));
        }
        if !in_flight {
            let record = records.remove_packet(pn, SentPacketState::Retired).unwrap();
            records.take_frames(record.frame_range, drop);
        }
    }
    ...
    pub fn cancel_pending(&self, pn: u64, frames: &mut Vec<Frame<()>>) {
        let mut records = self.0.lock().unwrap();
        if let Some(record) = records.remove_packet(pn, SentPacketState::Failed) {
            ...
            records.take_frames(record.frame_range, |frame| frames.push(frame.into()));
        }
    }
    ...
    pub fn acknowledge(
        ...
        for range in ranges.into_iter().rev() {
            while let Some((&pn, _)) = records.packets.range(range.clone()).next() {
                ...
                acknowledged.push(record.generation);
            }
        }
        records.reclaim();
        Ok(acknowledged)
...
mod tests {
    ...
    fn frame(value: u32) -> Frame<()> {
        Frame::MaxData(qbase::frame::MaxDataFrame::new(value.into()))
    }
    ...
    #[test]
    fn pending_path_frames_stay_with_sender_and_never_enter_loss_recovery() {
        let records = ArcSendJournal::default();
        let challenge = qbase::frame::PathChallengeFrame::random();
        let response = qbase::frame::PathResponseFrame::from(challenge);
        let mut scratch = vec![
        ...
        assert!(matches!(scratch.as_slice(),
            [Frame::PathChallenge(c), Frame::PathResponse(r)]
            if *c == challenge && *r == response));
        assert_eq!(scratch.as_ptr(), allocation);
        records.mark_sent(0, true, Duration::from_secs(1), Duration::from_secs(3));
        scratch.clear();
        records.mark_lost(&mut [0].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        records.on_tick(Instant::now(), |frame| scratch.push(frame.clone().into()));
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 10));
    ...
    fn recovery_keeps_data_headers_and_moves_tokens_without_copying() {
        use qbase::frame::{
            CryptoFrame, DatagramFrame, NewTokenFrame, PaddingFrame, PingFrame, StreamFrame,
        };
        let records = ArcSendJournal::default();
    ...
    async fn reverse_submission_and_ack_order_preserve_frame_ranges() {
        ...
        assert_eq!(scratch.as_ptr(), allocation);
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 120));
        assert!(records.0.lock().unwrap().frames.is_empty());
    ...
    fn batch_ack_reclaims_prefix_without_moving_pending_frame_indices() {
        ...
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(frame)] if frame.max_data() == 30));
        assert!(records.0.lock().unwrap().frames.is_empty());
    }
    ...
    async fn concurrent_batches_keep_each_packets_frames_together() {
        ...
        for pn in (0..400).rev() {
            ...
            let [Frame::MaxData(a), Frame::MaxData(b)] = scratch.as_slice() else {
                panic!()
            };
    ...
    async fn terminal_slots_are_reclaimed_only_when_the_live_prefix_finishes() {
        ...
        records.cancel_pending(2, &mut scratch);
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 2));
        scratch.clear();
        records.mark_lost(&mut [3].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        records.on_tick(Instant::now(), |frame| scratch.push(frame.clone().into()));
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 3));
        scratch.clear();
        ...
        records.cancel_pending(0, &mut scratch);
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 0));
        scratch.clear();
        {
            let journal = records.0.lock().unwrap();
        ...
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 4));
        assert!(records.0.lock().unwrap().frames.is_empty());
    }
    ...
    async fn loss_retains_frames_for_late_ack_and_expiration_reclaims_storage() {
        ...
        records.mark_lost(&mut [0, 1].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        records.on_tick(Instant::now(), |frame| scratch.push(frame.clone().into()));
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 10));
        ...
        records.mark_sent(2, true, Duration::from_secs(1), Duration::from_secs(3));
        records.mark_lost(&mut [2].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        records.on_tick(Instant::now(), |frame| scratch.push(frame.clone().into()));
        scratch.clear();
    ...
    async fn repeated_loss_never_requeues_retained_packets_or_extends_retention() {
        ...
        records.on_tick(Instant::now(), |frame| scratch.push(frame.clone().into()));
        assert!(
            matches!(scratch.as_slice(), [Frame::MaxData(a), Frame::MaxData(b)]
        ...
        records.mark_lost(&mut [1].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        records.on_tick(Instant::now(), |frame| scratch.push(frame.clone().into()));
        assert!(
    ...
    async fn timer_and_loss_feedback_schedule_each_packet_once() {
        ...
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 2));
        scratch.clear();
        records.mark_lost(&mut [0, 2].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        records.on_tick(now + Duration::from_secs(1), |frame| {
            scratch.push(frame.clone().into())
    ...
    async fn shared_handles_distinguish_cancelled_pending_and_sent_packets() {
        ...
        assert!(matches!(frames.as_slice(), [Frame::MaxData(f)] if f.max_data() == 10));
        frames.clear();
    ...
    async fn deadlines_order_by_time_and_keep_packets_with_equal_times() {
        ...
        assert_eq!(acknowledged_values(&records, 0), [0]);
        records.mark_lost(&mut [3, 3].into_iter(), |frame| {
            scratch.push(frame.clone().into())
        });
        {
            let journal = records.0.lock().unwrap();
    ...
    async fn equal_retransmission_and_retirement_deadlines_deliver_data_once() {
        ...
        records.on_tick(now, |frame| scratch.push(frame.clone().into()));
        assert!(matches!(scratch.as_slice(), [Frame::MaxData(f)] if f.max_data() == 10));
        scratch.clear();
        records.on_tick(now, |frame| scratch.push(frame.clone().into()));
        assert!(scratch.is_empty());
