...
mod tests {
    ...
    fn packet(
        pn: u64,
        keys: &OneRttKeys,
    ) -> Result<crate::send::packet::PendingPacket, PacketError> {
    ...
    async fn one_rtt_wait_yields_shared_material_and_reports_retirement() {
        ...
        assert_eq!(packet(0, &material).unwrap().generation, 1);
        keys.retire();
    ...
    fn first_update_needs_permission_and_later_updates_need_current_generation_ack() {
        ...
        assert_eq!(packet(1, &keys).unwrap().generation, 1);
        keys.on_ack(0);
        assert!(
            keys.update().is_err(),
    ...
    async fn key_updates_preserve_reordering_and_do_not_double_advance_sealing() {
        ...
        for generation in 1..=3 {
            let old = packet(generation * 10 - 1, &sending).unwrap().bytes;
            sending.on_ack(generation - 1);
            sending.update().unwrap();
    ...
    async fn peer_can_update_again_while_local_update_is_disallowed() {
        ...
        let oldest = packet(98, &sending).unwrap().bytes;
        assert_eq!(open(&server, packet(99, &sending).unwrap().bytes), Some(99));
        sending.update().unwrap();
        let delayed = packet(100, &sending).unwrap().bytes;
        assert_eq!(
            open(&server, packet(101, &sending).unwrap().bytes),
            Some(101)
        );
        ...
        *forged.last_mut().unwrap() ^= 1;
        ...
        receiving.on_ack(1);
        assert!(receiving.update().is_err());
        assert_eq!(packet(8, &receiving).unwrap().generation, 2);
        receiving.on_ack(2);
        receiving.update().unwrap();
        assert_eq!(
            open(&server, oldest),
    ...
    async fn local_update_keeps_old_opening_until_peer_uses_the_new_pair() {
        ...
        let old = packet(0, &sending).unwrap().bytes;
        let late_old = packet(1, &sending).unwrap().bytes;
        receiving.update().unwrap();
        tokio::time::advance(Duration::from_secs(4)).await;
        assert_eq!(open(old), Some(0));
        sending.update().unwrap();
        assert_eq!(open(packet(2, &sending).unwrap().bytes), Some(2));
        assert!(
            receiving.update().is_err(),
        ...
        assert_eq!(open(packet(3, &sending).unwrap().bytes), Some(3));
    }
    ...
    fn confidentiality_limit_updates_when_permitted_and_fails_when_not() {
        ...
        assert_eq!(packet(0, &keys).unwrap().generation, 1);
        {
            let mut packets = keys.packets.lock().unwrap();
            packets.sealed_count = packets.keys.back().unwrap().sealing.confidentiality_limit();
        }
    ...
    fn authentication_failures_enforce_the_integrity_limit() {
        ...
        let mut forged = packet(0, &sending).unwrap().bytes;
